...
bool FFMpegVideoEncoderPriv::createFile( const QString& fileName )
{
	...
	if ( m_aplayer )
	{
        ...
        av_opt_set_chlayout( audioResampleCtx, "in_channel_layout", &audioCodecCtx->ch_layout, 0 );
        av_opt_set_chlayout( audioResampleCtx, "out_channel_layout", &audioCodecCtx->ch_layout, 0 );
...
int FFMpegVideoEncoderPriv::encodeMoreAudio()
{
    ...
    while ( avcodec_receive_frame( m_aplayer->aCodecCtx, decodedAudioFrame ) >= 0 )
    {
        ...
        resampledAudioframe->ch_layout = audioCodecCtx->ch_layout;
